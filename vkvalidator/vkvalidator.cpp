...
const int MISSING_DRIVER_REGISTRY = -3;
const int MISSING_DRIVER_JSON = -4;
const int MISSING_DRIVER_LIB = -5;
...
// Try again in home folder
        ...
        case MISSING_DRIVER_LIB:
            ...
            break;
        default:
            std::cout << "ERROR: Failures occurred during validation"
                      << std::endl;
            break;
        }
        err_val = e;
...
void StartOutput(std::string output) {
    ...
    global_items.html_file_stream
        ...
        << "        html { " << std::endl
        << "            background: url(images/star_field.png) no-repeat "
           "center center fixed;"
        << std::endl
        << "            -webkit-background-size: cover;" << std::endl
        << "            -moz-background-size: cover;" << std::endl
        << "            -o-background-size: cover;" << std::endl
        << "            background-size: cover;" << std::endl
        << "            background-color: black;" << std::endl
        << "        }" << std::endl
        << "" << std::endl
        << "        #page-wrap { width: 400px; margin: 50px auto; padding: "
        ...
        << std::endl
        << "<BODY>" << std::endl
        << "<center><img src=\"images/vkvalidator.png\" alt=\"Vulkan "
           "Validator\" align=\"middle\"></img></center>"
        << std::endl
        << std::endl
        << "    <br />" << std::endl;
}
...
void BeginSection(std::string section) {
    global_items.html_file_stream << "    <H1><font color=\"Silver\">"
                                  << section << "</font></H1>" << std::endl
                                  << "    <HR/>" << std::endl;
}

void EndSection() { global_items.html_file_stream << "    <BR/>" << std::endl; }
...
void PrintBeginTableRow() {
    global_items.html_file_stream << "        <tr bgcolor=\"CornSilk\">"
                                  << std::endl;
}
...
void PrintSystemInfo(void) {
    ...
    if (0 !=
        GetEnvironmentVariableA("PROGRAMFILES(X86)", generic_string, 1023)) {
        PrintBeginTableRow();
        PrintTableElement("");
    ...
    int bytes = GetModuleFileName(NULL, generic_string, 1023);
    if (0 < bytes) {
        std::string exe_location = generic_string;
        exe_directory = exe_location.substr(0, exe_location.rfind("\\"));
        PrintBeginTableRow();
        PrintTableElement("Exe Directory");
        PrintTableElement(exe_directory);
        PrintEndTableRow();
    } else {
        exe_directory = "";
    }
    ...
    if (0 != GetCurrentDirectoryA(1023, generic_string)) {
        ...
        PrintBeginTableRow();
        PrintTableElement("Excuting in");
        PrintTableElement(generic_string);
        PrintEndTableRow();

        // If directories don't match, then we need to copy
        // the images folder to the new location so the HTML
        // file looks correct.
        if (exe_directory.size() != cur_directory.size() ||
            exe_directory.compare(cur_directory)) {
            cur_directory += "\\images\\";
            exe_directory += "\\images";
            DWORD dwAttrib = GetFileAttributes(cur_directory.c_str());
            if (!(dwAttrib != INVALID_FILE_ATTRIBUTES &&
                 dwAttrib & FILE_ATTRIBUTE_DIRECTORY)) {
                sprintf(generic_string, "mkdir %s > NUL", cur_directory.c_str());
                system(generic_string);
            }
            sprintf(generic_string, "copy /y %s %s > NUL", exe_directory.c_str(),
                cur_directory.c_str());
            system(generic_string);
        }
    } else {
    ...
    sprintf(generic_string, "%d.%d", VALIDATOR_MAJOR_VERSION,
            VALIDATOR_MINOR_VERSION);
    PrintBeginTableRow();
    ...
    sprintf(generic_string, "%d.%d.%d", major, minor, patch);
                ...
                if (!root["ICD"].isNull()) {
                    ...
                    if (!root["ICD"]["library_path"].isNull()) {
                        ...
                        if (GenerateLibraryPath(
                            ...
                            sprintf(generic_string, "Failed to find driver %s "
...
void PrintSystemInfo(void) {
    ...
    ssize_t len = ::readlink("/proc/self/exe", generic_string, 1023);
    if (0 < len) {
        std::string exe_location = generic_string;
        exe_directory = exe_location.substr(0, exe_location.rfind("/"));
        PrintBeginTableRow();
    ...
    if (getcwd(generic_string, 1023) != NULL) {
        ...
        if (exe_directory.size() != cur_directory.size() ||
            exe_directory.compare(cur_directory)) {
            exe_directory += "/images";
            sprintf(generic_string, "/bin/cp -Rf %s %s", exe_directory.c_str(),
                    cur_directory.c_str());
            system(generic_string);
        }
    } else {
